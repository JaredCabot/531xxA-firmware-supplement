...
- `531xx_eeprom_tool.py` - offline editor for a saved EEPROM (NVRAM) dump. Reads
  and decodes the store, changes the Channel 3 option, and can factory-blank the
  image to reset a forgotten calibration security code.
...
- Firmware image archives for the three counters (`*.zip`), provided for
  reference and for use with the EEPROM tool.
